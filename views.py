...
def build_backup_restore_view(deps):
    ...
    def backup_restore():
        ...
        if request.method == "POST":
            ...
            shutil.copy2(db_path, os.path.join(backup_dir, f"before-restore-{datetime.now().strftime('%Y%m%d-%H%M%S')}.db"))
            shutil.copy2(restore_path, db_path)
            flash("تمت استعادة قاعدة البيانات. أعد تشغيل البرنامج إذا لزم الأمر.", "success")
            return redirect(url_for("backup_restore"))
