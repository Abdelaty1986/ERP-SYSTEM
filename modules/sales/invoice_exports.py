...
def _sales_invoice_export_query(created_by_expr, join_users_sql, customer_tax_expr, where_sql):
    return f"""
        ...
        SELECT
            ...
            COALESCE(si.subtotal, si.total, 0) AS subtotal,
            COALESCE(si.withholding_total, si.withholding_amount, 0) AS discount_amount,
            COALESCE(si.vat_total, si.tax_amount, 0) AS tax_amount,
            COALESCE(si.net_total, si.grand_total, 0) AS net_amount,
