...
class ProviderScoringMemory:
    ...
    def build(self):
        ...
        for provider in provider_names:
            validation_entry = (validation.get("providers") or {}).get(provider, {})
            runtime_entry = (runtime_trust.get("providers") or {}).get(provider, {})
            model_entry = (model_trust.get("providers") or {}).get(provider, {})

            validation_score = float(validation_entry.get("confidence_score", 0.0) or 0.0)
            ...
            inconsistency = self.detect_inconsistency(
            ...
            provider_scores[provider] = {
                "validation_state": validation_entry.get("validation_state"),
                "runtime_state": runtime_entry.get("last_state"),
                "model_trust_state": model_entry.get("trust_state"),
                "configured_model": validation_entry.get("model")
                ...
                "combined_observation_score": combined_score,
                "ready_in_static_validation": bool(validation_entry.get("ready")),
                "current_runtime_missing_credentials": (
                    runtime_entry.get("last_state") == "missing_credentials"
                ),
                "review_required": bool(inconsistency),
                "recommendation_scope": "routing_memory_only",
            }

        ranked_providers = sorted(
            provider_scores,
            key=lambda name: provider_scores[name]["combined_observation_score"],
            reverse=True,
        )
        ...
        result = {
            ...
            **locked_controls,
            ...
            "recommended_provider_for_review": (
                ranked_providers[0] if ranked_providers else None
            ),
            "prior_routing_recommendation": (
                routing_hud.get("adaptive_routing_decision", {})
                .get("recommended_provider")
            ),
            "agent_learning_state": (
                agent_memory.get("learning_assessment", {}).get("learning_state")
            ),
            "review_flags": review_flags,
            "learning_assessment": {
                "learning_state": (
                    "provider_scores_need_review"
                    if review_flags or unreadable_sources
                    else "stable_provider_scoring_memory"
                ),
    ...
    def detect_inconsistency(self, validation_entry, runtime_entry, model_entry):
        if validation_entry.get("ready") and runtime_entry.get("last_state") == "missing_credentials":
            return "static_validation_ready_but_runtime_credentials_missing"
        if model_entry.get("trust_state") == "trusted" and runtime_entry.get("last_state") == "missing_credentials":
            return "model_trust_high_but_runtime_credentials_missing"
        return None
...
if __name__ == "__main__":
    output = ProviderScoringMemory().build()
