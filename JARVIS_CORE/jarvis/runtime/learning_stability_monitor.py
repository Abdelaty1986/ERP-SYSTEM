...
SOURCES = {
    ...
    "runtime_continuity_memory": LOGS_DIR / "runtime_continuity_memory.json",
}
...
class LearningStabilityMonitor:
    ...
    def build(self):
        ...
        continuity = sources["runtime_continuity_memory"].get("data") or {}
        ...
        unsafe_layers = [
            ...
            if state.get("execution_allowed") or state.get("apply_allowed")
        ]

        stable_layers = [
            name for name, state in layer_states.items()
            if "stable" in str(state.get("learning_state"))
            or "ready" in str(state.get("learning_state"))
            or "strong" in str(state.get("learning_state"))
        ]

        continuity_state = continuity.get("continuity", {}).get("state")
        operational_mode = continuity.get("identity", {}).get("operational_mode")

        safety_stability = (
            "stable"
            if not unsafe_layers and not blocked_capabilities and not unreadable_sources
            else "unstable"
        )
        learning_stability = (
            "watch"
            if review_items
            else ("stable" if safety_stability == "stable" else "unstable")
        )
        ...
        result = {
            ...
            **locked_controls,
            ...
            "learning_assessment": {
                "learning_state": (
                    "learning_stability_watch"
                    if learning_stability == "watch"
                    else "stable_learning_runtime"
                ),
...
if __name__ == "__main__":
    output = LearningStabilityMonitor().build()
