...
class RuntimeLearningSummary:
    ...
    def build(self):
        ...
        for name, payload in sources.items():
            ...
            learning_state = (
                data.get("learning_assessment", {}).get("learning_state")
                or data.get("learning_state")
                or data.get("result")
            )
            layer_states[name] = {
                "available": payload.get("exists") and data != {},
        ...
        review_items.extend(
            routing_memory.get("recommendations", {})
            .get("provider", {})
            .get("review_flags", [])
        )
        ...
        result = {
            ...
            **locked_controls,
            ...
            "learning_summary": {
                "engineering_learning": layer_states.get(
                    "engineering_learning_memory", {}
                ).get("learning_state"),
                "agent_learning": layer_states.get(
                    "agent_performance_memory", {}
                ).get("learning_state"),
                "provider_learning": layer_states.get(
                    "provider_scoring_memory", {}
                ).get("learning_state"),
                "routing_learning": layer_states.get(
            ...
            "learning_assessment": {
                "learning_state": (
                    "runtime_learning_review_required"
                    if unique_review_items or unreadable_sources
                    else "stable_runtime_learning_summary"
                ),
...
if __name__ == "__main__":
    output = RuntimeLearningSummary().build()
