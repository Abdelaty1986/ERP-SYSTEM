...
class SafeLearningHudReport:
    ...
    def build(self):
        ...
        provider_recommendation = (
            routing.get("recommendations", {}).get("provider", {})
        )
        agent_recommendation = (
            routing.get("recommendations", {}).get("agent", {})
        )
        ...
        for item in summary.get("review_items", []):
            warnings.append({
                ...
                "subject": item.get("provider"),
            })
        if unreadable_sources:
            warnings.append({
                "type": "source_read_error",
                "message": "One or more learning sources could not be read.",
                "subject": unreadable_sources,
            })
        ...
        hud = {
            ...
            "next_runtime_allowed": (
                stability.get("monitor_decision", {})
                .get("next_runtime_allowed", False)
            ),
            ...
            "sections": {
                "engineering_learning": {
                    "state": (
                        engineering.get("learning_assessment", {})
                        .get("learning_state")
                    ),
                    "active_strategy": (
                        engineering.get("engineering_signals", {})
                        .get("active_strategy")
                    ),
                    "active_goal": (
                        engineering.get("engineering_signals", {})
                ...
                "agent_performance": {
                    "state": (
                        agents.get("learning_assessment", {})
                        .get("learning_state")
                    ),
                ...
                "provider_scoring": {
                    "state": (
                        providers.get("learning_assessment", {})
                        .get("learning_state")
                    ),
                    "provider_count": providers.get("provider_count"),
                    "recommended_provider_for_review": (
                        providers.get("recommended_provider_for_review")
                    ),
                },
                "routing_recommendations": {
                    "state": (
                        routing.get("learning_assessment", {})
                        .get("learning_state")
                    ),
                    "provider_decision_state": provider_recommendation.get("decision_state"),
                    "agent_decision_state": agent_recommendation.get("decision_state"),
                },
            },
            "warnings": warnings,
            "locks": {
        ...
        result = {
            ...
            **locked_controls,
            ...
            "learning_assessment": {
                "learning_state": (
                    "safe_learning_hud_review_visible"
                    if warnings
                    else "safe_learning_hud_stable"
                ),
                "safe_to_display_in_hud": not unreadable_sources,
...
if __name__ == "__main__":
    output = SafeLearningHudReport().build()
