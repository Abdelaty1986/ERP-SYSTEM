    ...
    def sales_invoice_from_delivery():
        ...
        if request.method == "POST":
            ...
            if not date_value:
                ...
                flash("اختر إذن صرف واحدا على الأقل.", "danger")
                ...
                flash("الفاتورة الآجلة تحتاج عميلا مرتبطا بأمر البيع.", "danger")
            else:
                try:
                ...
                total = sum(row[7] for row in deliveries)
                cost_total = sum(row[8] for row in deliveries)
                tax_amount = sum(row[10] for row in deliveries)
                grand_total = sum(row[11] for row in deliveries)
                withholding_rate = _customer_withholding(cur, deliveries[0][3])
                ...
                cur.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """,
                    (date_value, due_date, doc_no, first[3], first[4], 1, total, total, cost_total, first[9], tax_amount, withholding_rate, withholding_amount, grand_total, payment_type, journal_id, tax_journal_id, withholding_journal_id, None, "posted", first[2], first[0], po_ref, gr_ref or delivery_refs, notes),
                )
                invoice_id = cur.lastrowid
                for row in deliveries:
                    cur.execute(
                        "INSERT INTO sales_invoice_lines(invoice_id,product_id,quantity,unit_price,total,cost_total) VALUES (?,?,?,?,?,?)",
                        (invoice_id, row[4], row[5], row[6], row[7], row[8]),
                    )
        ...
        conn.close()
        return render_template("sales_invoice_from_delivery.html", open_deliveries=open_deliveries, selected_order_id=selected_order_id, selected_delivery_no=selected_delivery_no, selected_delivery_ids=selected_delivery_ids)
    ...
    def financial_sales():
        ...
        if request.method == "POST":
            ...
            if not date_value:
                ...
                flash("أدخل بندا واحدا على الأقل بقيمة صحيحة.", "danger")
            elif amount <= 0:
    ...
    def purchase_invoice_from_receipt():
        ...
        if request.method == "POST":
            ...
            if not date_value or not supplier_invoice_no or not supplier_invoice_date:
                ...
                flash("اختر إذن إضافة/استلام واحدا على الأقل.", "danger")
            elif any(row[12] for row in receipts):
                flash("يوجد إذن إضافة/استلام تم عمل فاتورة له من قبل.", "danger")
                ...
                total = sum(row[8] for row in receipts)
                tax_amount = sum(row[10] for row in receipts)
                grand_total = sum(row[11] for row in receipts)
                withholding_rate = _supplier_withholding(cur, receipts[0][4])
                withholding_amount = round(grand_total * withholding_rate / 100, 2)
                first = receipts[0]
                ...
                cur.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """,
                    (date_value, doc_no, supplier_invoice_no, supplier_invoice_date, due_date, first[4], first[5], 1, total, total, first[9], tax_amount, withholding_rate, withholding_amount, grand_total, payment_type, journal_id, tax_journal_id, withholding_journal_id, notes, "posted", first[3], first[0]),
                )
                invoice_id = cur.lastrowid
                for row in receipts:
                    cur.execute(
                        "INSERT INTO purchase_invoice_lines(invoice_id,product_id,quantity,unit_price,total) VALUES (?,?,?,?,?)",
                        (invoice_id, row[5], row[6], row[7], row[8]),
                    )
