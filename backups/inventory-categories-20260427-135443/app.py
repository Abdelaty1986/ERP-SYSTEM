...
def excel_response(filename, headers, rows, title="تقرير"):
    def _cell(value):
...
def post_sales_invoice(cur, invoice_id):
    ...
    if not product or product[1] < quantity:
        raise ValueError(f"رصيد الصنف لا يكفي لترحيل فاتورة بيع #{invoice_id}.")
    debit_code = "1300" if payment_type == "credit" else "1100"
    journal_id = create_auto_journal(cur, date_value, f"فاتورة بيع - {product[0]}", debit_code, "4100", total)
    tax_journal_id = create_auto_journal(cur, date_value, f"ضريبة قيمة مضافة على فاتورة بيع - {product[0]}", debit_code, "2200", tax_amount) if tax_amount > 0 else None
    ...
    cogs_journal_id = create_auto_journal(cur, date_value, f"تكلفة بضاعة مباعة - {product[0]}", "6100", "1400", cost_total) if cost_total > 0 else None
    ...
    cur.execute(
        ...
        VALUES (?,?,?,?,?,?,?)
        """,
        (date_value, product_id, "out", -quantity, "sale", invoice_id, "ترحيل فاتورة بيع"),
    )
    cur.execute(
        """
...
def post_purchase_invoice(cur, invoice_id):
    ...
    product = cur.fetchone()
    if not product:
        raise ValueError(f"الصنف غير موجود في فاتورة مورد #{invoice_id}.")
    credit_code = "2100" if payment_type == "credit" else "1100"
    journal_id = create_auto_journal(cur, date_value, f"فاتورة شراء - {product[0]}", "1400", credit_code, total)
    tax_journal_id = create_auto_journal(cur, date_value, f"ضريبة قيمة مضافة على فاتورة شراء - {product[0]}", "1500", credit_code, tax_amount) if tax_amount > 0 else None
    ...
    cur.execute(
        ...
        VALUES (?,?,?,?,?,?,?)
        """,
        (date_value, product_id, "in", quantity, "purchase", invoice_id, "ترحيل فاتورة مورد"),
    )
    cur.execute(
...
def unpost_purchase_invoice(cur, invoice_id):
    ...
    if not stock or stock[0] < quantity:
        raise ValueError(f"رصيد الصنف لا يكفي لفك ترحيل فاتورة مورد #{invoice_id}.")
    cur.execute("UPDATE products SET stock_quantity=stock_quantity-? WHERE id=?", (quantity, product_id))
...
def post_voucher(cur, table, source_type, voucher_id):
    ...
    label = "سند قبض من" if source_type == "receipts" else "سند صرف إلى"
    cur.execute(
    ...
    party = cur.fetchone()
    if not party:
        raise ValueError(f"الطرف غير موجود في المستند #{voucher_id}.")
...
def reverse_journal(cur, journal_id, date, reason):
    ...
    cur.execute(
        ...
        VALUES (?,?,?,?,?,'posted','auto')
        """,
        (date, f"قيد عكسي - {description} - {reason}", credit_id, debit_id, amount),
    )
    return cur.lastrowid
...
def customers():
    return party_page("customers", "العملاء", "تم إضافة العميل بنجاح.")
...
def suppliers():
    return party_page("suppliers", "الموردون", "تم إضافة المورد بنجاح.")
    ...
    if request.method == "POST":
        ...
        if not name:
            ...
            try:
                ...
                conn.commit()
                flash("تم إضافة مركز التكلفة.", "success")
                return redirect(url_for("cost_centers"))
            except sqlite3.IntegrityError:
                flash("كود مركز التكلفة مستخدم بالفعل.", "danger")
...
def document_sequences():
    ...
    if request.method == "POST":
        ...
        if not doc_type or not prefix or next_number <= 0:
            flash("راجع بيانات مسلسل المستند.", "danger")
        else:
            cur.execute(
                """
            ...
            conn.commit()
            flash("تم حفظ مسلسل المستند.", "success")
...
def sales_multi():
    ...
    if request.method == "POST":
        ...
        if not date_value or not lines:
            flash("التاريخ وبند واحد على الأقل مطلوبان.", "danger")
            ...
            flash("رصيد أحد الأصناف لا يكفي.", "danger")
        else:
            try:
            ...
            debit_code = "1300" if payment_type == "credit" else "1100"
            journal_id = create_auto_journal(cur, date_value, f"فاتورة بيع متعددة البنود {doc_no}", debit_code, "4100", total)
            ...
            cogs_journal_id = create_auto_journal(cur, date_value, f"تكلفة بضاعة مباعة {doc_no}", "6100", "1400", cost_total) if cost_total > 0 else None
            cur.execute(
                """
            ...
            for line in lines:
                ...
                cur.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?)
                    """,
                    (date_value, line["product_id"], "out", -line["quantity"], "sale", invoice_id, f"فاتورة بيع {doc_no}"),
                )
            log_action(cur, "create", "sales_invoice", invoice_id, doc_no)
            conn.commit()
            conn.close()
            rebuild_ledger()
            flash(f"تم حفظ وترحيل فاتورة البيع {doc_no}.", "success")
    ...
    conn.close()
    return render_template("multi_invoice.html", title="فاتورة بيع متعددة البنود", customers=customers_rows, suppliers=[], products=product_rows, action_url=url_for("sales_multi"), party="customer")
...
def purchases_multi():
    ...
    if request.method == "POST":
        ...
        if not date_value or not supplier_invoice_no or not supplier_invoice_date or not lines:
            flash("بيانات الفاتورة وبند واحد على الأقل مطلوبة.", "danger")
            ...
            credit_code = "2100" if payment_type == "credit" else "1100"
            journal_id = create_auto_journal(cur, date_value, f"فاتورة مورد متعددة البنود {doc_no}", "1400", credit_code, total)
            ...
            for line in lines:
                ...
                cur.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?)
                    """,
                    (date_value, line["product_id"], "in", line["quantity"], "purchase", invoice_id, f"فاتورة مورد {doc_no}"),
                )
            log_action(cur, "create", "purchase_invoice", invoice_id, doc_no)
            conn.commit()
            conn.close()
            rebuild_ledger()
            flash(f"تم حفظ وترحيل فاتورة المورد {doc_no}.", "success")
    ...
    product_rows = cur.fetchall()
    conn.close()
    return render_template("multi_invoice.html", title="فاتورة مورد متعددة البنود", customers=[], suppliers=suppliers_rows, products=product_rows, action_url=url_for("purchases_multi"), party="supplier")
...
@app.route("/sales/<int:id>/export-excel")
@login_required
@permission_required("sales")
...
@app.route("/purchases/<int:id>/export-excel")
@login_required
@permission_required("purchases")
...
def _products_v2():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            purchase_price = 0
            sale_price = 0
            flash("أسعار المنتج يجب أن تكون أرقاما.", "danger")
            supplier_id = None
        if supplier_id:
            cur.execute("SELECT 1 FROM suppliers WHERE id=?", (supplier_id,))
            if not cur.fetchone():
    ...
    if request.method == "POST":
        ...
        if not date_value or not invoice or not lines:
            ...
            for product_id, quantity, option in lines:
                ...
                cur.execute("UPDATE products SET stock_quantity=stock_quantity+? WHERE id=?", (quantity, product_id))
                cur.execute("INSERT INTO inventory_movements(date,product_id,movement_type,quantity,reference_type,reference_id,notes) VALUES (?,?,?,?,?,?,?)", (date_value, product_id, "return_in", quantity, "sales_return", return_id, notes or "مردود بيع"))
    ...
    if request.method == "POST":
        ...
        if not date_value or not invoice or not lines:
            ...
            for product_id, quantity, option in lines:
                ...
                cur.execute("UPDATE products SET stock_quantity=stock_quantity-? WHERE id=?", (quantity, product_id))
                cur.execute("INSERT INTO inventory_movements(date,product_id,movement_type,quantity,reference_type,reference_id,notes) VALUES (?,?,?,?,?,?,?)", (date_value, product_id, "return_out", -quantity, "purchase_return", return_id, notes or "مردود مشتريات"))
...
def _customer_statement_v2(id):
    ...
    for date, invoice_id, total, payment_type, status, cancel_reason in cur.fetchall():
        ...
        if payment_type == "credit":
            entries.append((date, f"فاتورة بيع آجلة #{invoice_id}{suffix}", total, 0, display_status))
            if status == "cancelled":
                entries.append((date, f"إلغاء فاتورة بيع آجلة #{invoice_id}", 0, total, "إلغاء"))
        else:
            entries.append((date, f"فاتورة بيع نقدية #{invoice_id}{suffix}", total, 0, display_status))
            entries.append((date, f"تحصيل نقدي لفاتورة #{invoice_id}{suffix}", 0, total, display_status))
    cur.execute(
        """
    ...
    for date, return_id, total, payment_type, product_name in cur.fetchall():
        if payment_type == "credit":
            entries.append((date, f"مردود مبيعات #{return_id} - {product_name}", 0, total, "مرحل"))
        else:
            entries.append((date, f"مردود مبيعات نقدي #{return_id} - {product_name}", 0, total, "مرحل"))
            entries.append((date, f"رد نقدية عن مردود #{return_id}", total, 0, "مرحل"))
    ...
    for date, voucher_id, amount, notes, status, cancel_reason in cur.fetchall():
        ...
        if status == "cancelled":
            entries.append((date, f"إلغاء سند قبض #{voucher_id}", amount, 0, "إلغاء"))
    entries.sort(key=lambda row: row[0])
    debit = sum(row[2] for row in entries)
    credit = sum(row[3] for row in entries)
    balance = debit - credit
    conn.close()
    return render_template("party_statement.html", title=f"كشف حساب العميل: {customer[0]}", company=company, party_name=customer[0], party_type="عميل", rows=entries, debit=debit, credit=credit, balance=balance, balance_label="مدين" if balance > 0 else "دائن")
...
def _supplier_statement_v2(id):
    ...
    for date, invoice_id, total, payment_type, status, cancel_reason in cur.fetchall():
        ...
        suffix = f" - سبب الإلغاء: {cancel_reason}" if status == "cancelled" and cancel_reason else ""
        if payment_type == "credit":
            entries.append((date, f"فاتورة شراء آجلة #{invoice_id}{suffix}", 0, total, display_status))
        else:
            entries.append((date, f"فاتورة شراء نقدية #{invoice_id}{suffix}", 0, total, display_status))
            entries.append((date, f"سداد نقدي لفاتورة #{invoice_id}{suffix}", total, 0, display_status))
    cur.execute(
        """
    ...
    for date, return_id, total, payment_type, product_name in cur.fetchall():
        if payment_type == "credit":
            entries.append((date, f"مردود مشتريات #{return_id} - {product_name}", total, 0, "مرحل"))
        else:
            entries.append((date, f"مردود مشتريات نقدي #{return_id} - {product_name}", total, 0, "مرحل"))
            entries.append((date, f"استرداد نقدية عن مردود #{return_id}", 0, total, "مرحل"))
    ...
    for date, voucher_id, amount, notes, status, cancel_reason in cur.fetchall():
        ...
        if status == "cancelled":
            entries.append((date, f"إلغاء سند صرف #{voucher_id}", 0, amount, "إلغاء"))
    entries.sort(key=lambda row: row[0])
    debit = sum(row[2] for row in entries)
    credit = sum(row[3] for row in entries)
    balance = credit - debit
    conn.close()
    return render_template("party_statement.html", title=f"كشف حساب المورد: {supplier[0]}", company=company, party_name=supplier[0], party_type="مورد", rows=entries, debit=debit, credit=credit, balance=balance, balance_label="دائن" if balance > 0 else "مدين")
...
def _print_purchase_v2(id):
    ...
    return render_template(
        ...
        company=company,
        doc=doc,
        doc_type="مستند داخلي - تسجيل فاتورة مورد",
        party_label="المورد",
...
def _table_columns(cur, table_name):
    ...
    if not table_name or not _table_exists(cur, table_name):
        return set()
    cur.execute(f"PRAGMA table_info({table_name})")
    return {row[1] for row in cur.fetchall()}


def export_sales_invoices_excel_file():
    """Download posted customer invoices as XLSX.

    This is read-only: it does not upload to the tax portal and does not change
    invoice or e-invoice statuses.
    """
    conn = db()
    cur = conn.cursor()

    sales_cols = _table_columns(cur, "sales_invoices")
    customer_cols = _table_columns(cur, "customers")
    product_cols = _table_columns(cur, "products")
    line_table = "sales_invoice_lines" if _table_exists(cur, "sales_invoice_lines") else None
    line_cols = _table_columns(cur, line_table) if line_table else set()
    einvoice_cols = _table_columns(cur, "e_invoice_documents")

    if not sales_cols:
        conn.close()
        flash("جدول فواتير البيع غير موجود في قاعدة البيانات.", "danger")
        return redirect(url_for("e_invoices"))

    customer_name_expr = "COALESCE(c.name, '')" if "name" in customer_cols else "''"
    customer_tax_expr = "''"
    for col in ("tax_registration_number", "tax_number", "tax_card_number", "vat_number"):
        if col in customer_cols:
            customer_tax_expr = f"COALESCE(c.{col}, '')"
            break

    product_name_expr = "COALESCE(p.name, '')" if "name" in product_cols else "''"
    doc_no_expr = "COALESCE(s.doc_no, s.id)" if "doc_no" in sales_cols else "s.id"
    due_date_expr = "COALESCE(s.due_date, '')" if "due_date" in sales_cols else "''"
    payment_type_expr = "COALESCE(s.payment_type, '')" if "payment_type" in sales_cols else "''"
    status_expr = "COALESCE(s.status, '')" if "status" in sales_cols else "''"
    tax_rate_expr = "COALESCE(s.tax_rate, 0)" if "tax_rate" in sales_cols else "0"
    e_status_expr = "COALESCE(e.status, '')" if "status" in einvoice_cols else "''"
    e_uuid_expr = "COALESCE(e.eta_uuid, '')" if "eta_uuid" in einvoice_cols else "''"

    export_rows = []

    if line_table:
        line_quantity_expr = "COALESCE(l.quantity, 0)" if "quantity" in line_cols else "0"
        line_unit_price_expr = "COALESCE(l.unit_price, 0)" if "unit_price" in line_cols else "0"
        line_total_expr = (
            "COALESCE(l.total, COALESCE(l.quantity, 0) * COALESCE(l.unit_price, 0))"
            if "total" in line_cols
            else "COALESCE(l.quantity, 0) * COALESCE(l.unit_price, 0)"
        )
        line_product_join = (
            "LEFT JOIN products p ON p.id = l.product_id"
            if "product_id" in line_cols and product_cols
            else "LEFT JOIN products p ON 1=0"
        )
        posted_filter = "WHERE s.status='posted'" if "status" in sales_cols else ""

        cur.execute(
            f"""
            SELECT
                s.id,
                {doc_no_expr} AS doc_no,
                COALESCE(s.date, '') AS invoice_date,
                ...
                {line_quantity_expr} AS quantity,
                {line_unit_price_expr} AS unit_price,
                {line_total_expr} AS net_amount,
                {tax_rate_expr} AS tax_rate,
                ({line_total_expr} * {tax_rate_expr} / 100.0) AS tax_amount,
                ({line_total_expr} + ({line_total_expr} * {tax_rate_expr} / 100.0)) AS gross_amount,
                {e_status_expr} AS einvoice_status,
                {e_uuid_expr} AS eta_uuid
            FROM sales_invoices s
            LEFT JOIN customers c ON c.id = s.customer_id
            LEFT JOIN {line_table} l ON l.invoice_id = s.id
            {line_product_join}
            LEFT JOIN e_invoice_documents e
                ON e.document_type='sale'
                AND e.document_id=s.id
            {posted_filter}
            ORDER BY COALESCE(s.date, ''), s.id, l.id
            """
        )
        export_rows.extend(cur.fetchall())

    has_direct_product = {"product_id", "quantity", "unit_price"}.issubset(sales_cols)
    if has_direct_product:
        direct_total_expr = (
            "COALESCE(s.total, COALESCE(s.quantity, 0) * COALESCE(s.unit_price, 0))"
            if "total" in sales_cols
            else "COALESCE(s.quantity, 0) * COALESCE(s.unit_price, 0)"
        )
        posted_filter = "WHERE s.status='posted'" if "status" in sales_cols else ""
        exclude_multi = ""
        if line_table:
            exclude_multi = (
                ...
                if posted_filter
                else "WHERE NOT EXISTS (SELECT 1 FROM sales_invoice_lines l2 WHERE l2.invoice_id=s.id)"
            )
        ...
        cur.execute(
            ...
            SELECT
                ...
                {direct_total_expr} AS net_amount,
                {tax_rate_expr} AS tax_rate,
                COALESCE(s.tax_amount, ({direct_total_expr} * {tax_rate_expr} / 100.0)) AS tax_amount,
                COALESCE(s.grand_total, ({direct_total_expr} + ({direct_total_expr} * {tax_rate_expr} / 100.0))) AS gross_amount,
                {e_status_expr} AS einvoice_status,
                {e_uuid_expr} AS eta_uuid
            FROM sales_invoices s
            LEFT JOIN customers c ON c.id = s.customer_id
    ...
    for col_idx in range(10, 16):
        for column in ws.iter_cols(min_col=col_idx, max_col=col_idx, min_row=2):
            for cell in column:
...
@app.route("/e-invoices/sales/export-excel")
@login_required
@permission_required("e_invoices")
def export_sales_invoices_excel():
    return export_sales_invoices_excel_file()
